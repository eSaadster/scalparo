...
class MultiSymbolMomentumStrategy(BaseStrategy):
    ...
    def _initialize_indicators(self):
        ...
        for data in self.datas:
            atr = bt.indicators.ATR(data, period=288)
            weighted = (data.high + data.low + data.close * 2) / 4
            wavg = bt.indicators.SimpleMovingAverage(weighted, period=288)
            weekly_high = bt.indicators.Highest(data.high, period=2016)
            weekly_low = bt.indicators.Lowest(data.low, period=2016)
            vol_avg = bt.indicators.SimpleMovingAverage(data.volume, period=288)
            self.inds[data] = {
                'atr': atr,
                'wavg': wavg,
    ...
    def next(self):
        ...
        for data in self.datas:
            ...
            if not self._position_open(data):
                ...
                self.log(f'BUY CREATE {data._name}: Price {price:.2f}, Value {trade_value:.2f}')
                self.buy(data=data, size=size)
                ind['entry_price'] = price
                ind['size'] = size
                ind['orig_size'] = size
                ind['highest'] = price
                ind['trailing_stop'] = price - ind['atr'][0] * 1.8 if ind['atr'][0] else price * 0.99
                ...
                if profit_pct >= 0.3 and data.volume[0] > ind['vol_avg'][0] * 2:
                    ...
                    if add_value > 0:
                        ...
                        new_entry = (ind['entry_price'] * ind['size'] + price * add_size) / (ind['size'] + add_size)
                ...
                if sell_reason:
                    ...
                    if profit > 0:
                        ind['losses'] = 0
                        ind['trade_size_factor'] = min(1.0, ind['trade_size_factor'] * 1.5)
                        ind['halt_until'] = None
                    else:
                        ind['losses'] += 1
                        if ind['losses'] >= 5:
